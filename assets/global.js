...
class SliderComponent extends HTMLElement {
  ...
  update() {
    ...
    if (this.prevButton && this.sliderItemsToShow.length) {
    ...
    if (this.nextButton && this.sliderItemsToShow.length) {
...
class SlideshowComponent extends SliderComponent {
  constructor() {
    ...
    // ищем кнопки по всему компоненту, а не только в .slider-buttons
    this.sliderControlLinksArray = Array.from(
      this.querySelectorAll('.slider-counter__link')
    );
  ...
  setAutoPlay() {
    ...
    if (this.querySelector('.slideshow__autoplay')) {
      ...
      const prefersReduced = this.reducedMotion?.matches === true;
      const arrowClicked = !!this.announcementBarArrowButtonWasClicked;
      prefersReduced || arrowClicked ? this.pause() : this.play();
    }
  }
  ...
  update() {
    ...
    // защита от ошибки, если currentPage > кнопок
  ...
  focusOutHandling(event) {
    ...
    if (!this.sliderAutoplayButton || !this.autoplayButtonIsSetToPlay || focusedOnAutoplayButton) {
      this.play();
    }
  ...
  focusInHandling(event) {
    if (this.sliderAutoplayButton) {
      // стандартная логика с кнопкой play/pause
      const focusedOnAutoplayButton =
        event.target === this.sliderAutoplayButton || this.sliderAutoplayButton.contains(event.target);
      ...
      // кастом pills — кнопки нет
      if (this.announcementBarSlider && this.announcementBarSlider.contains(event.target)) {
        this.pause();
      } else {
        this.pause();
      }
